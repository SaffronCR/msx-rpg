...
void sf_draw_char(char character, int x, int y, char log_op)
{
	switch (character)
	{
		...
		case '<': sf_screen_copy(uplbx,uplby,	FONT_SIZE_X,FONT_SIZE_Y, x,y, FONT_PAGE, back_page, log_op); break;
		case '[': sf_screen_copy(dwnlbx,dwnlby,	FONT_SIZE_X,FONT_SIZE_Y, x,y, FONT_PAGE, back_page, log_op); break;
		case '_': sf_screen_copy(midbx,midby,	FONT_SIZE_X,FONT_SIZE_Y, x,y, FONT_PAGE, back_page, log_op); break;
		case '>': sf_screen_copy(uprbx,uprby,	FONT_SIZE_X,FONT_SIZE_Y, x,y, FONT_PAGE, back_page, log_op); break;
		case ']': sf_screen_copy(dwnrbx,dwnrby,	FONT_SIZE_X,FONT_SIZE_Y, x,y, FONT_PAGE, back_page, log_op); break;
		case '|': sf_screen_copy(latbx,latby,	FONT_SIZE_X,FONT_SIZE_Y, x,y, FONT_PAGE, back_page, log_op); break;
	}
}
...
void sf_draw_text(char *text, int x, int y, char text_color, char border_color)
{
	...
	while (*text)
	{
		...
		{
			// Color border.
			LMMV(x, y + back_page * 256, FONT_SIZE_X, FONT_SIZE_Y, border_color, LOGICAL_IMP);
